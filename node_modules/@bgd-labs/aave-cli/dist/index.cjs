...
__export(src_exports, {
  ...
  HUMAN_READABLE_STATE: () => HUMAN_READABLE_STATE,
  assetIndexesToAsset: () => assetIndexesToAsset,
  diffReports: () => diffReports,
  findAsset: () => findAsset,
  findPayloadsController: () => findPayloadsController,
  generateProposalReport: () => generateProposalReport,
...
function getContractName(contracts, address, chainId) {
  ...
  if (!contract)
    return `unknown contract name at \`${(0, import_viem8.getAddress)(address)}\`${flagKnownAddress(isAddrKnown)}`;
  ...
  if (contract.standards?.includes("eip1967") && contract.child_contracts?.[0].address) {
    return `${contractName} at \`${(0, import_viem8.getAddress)(address)}\`${flagKnownAddress(
      isAddrKnown
    )} with implementation ${getContractName(contracts, contract.child_contracts?.[0].address, chainId)}`;
  }
  return `${contractName} at \`${(0, import_viem8.getAddress)(address)}\`${flagKnownAddress(isAddrKnown)}`;
}
...
var checkLogs = {
  ...
  async checkProposal(proposal, sim, client) {
    ...
    const events = sim.transaction.transaction_info.logs?.reduce(
      (logs, log) => {
        const addr = (0, import_viem9.getAddress)(log.raw.address);
...
async function checkNoSelfdestruct(trustedAddrs, addr, client) {
  ...
  const [code, nonce] = await Promise.all([
    (0, import_actions.getBytecode)(client, { address: addr }),
    (0, import_actions.getTransactionCount)(client, { address: addr })
  ]);
    ...
    if (!simulation.transaction.status) {
      ...
      const stateDiffs = simulation.transaction.transaction_info.state_diff.reduce(
        (diffs, diff2) => {
          ...
          const addr = (0, import_viem10.getAddress)(diff2.raw[0].address);
...
async function checkVerificationStatus(sim, addr, client) {
  ...
  const code = await (0, import_actions2.getBytecode)(client, { address: addr });
  ...
  simulate = async (request, client) => {
    ...
    if (!request.state_objects[request.from]) {
      request.state_objects[request.from] = {
        balance: String((0, import_viem11.parseEther)("3"))
      };
    } else {
      request.state_objects[request.from].balance = String((0, import_viem11.parseEther)("3"));
    }
...
`
  ...
  deployCode = async (fork, filePath, from) => {
    const walletProvider = (0, import_viem11.createClient)({
      account: from || EOA,
      chain: { id: fork.forkNetworkId, name: "tenderly" },
      transport: (0, import_viem11.http)(fork.forkUrl)
    });
    const artifact = require(filePath);
    logInfo("tenderly", `deploying ${filePath}`);
    const hash = await (0, import_actions3.deployContract)(walletProvider, {
      abi: artifact.abi,
      bytecode: artifact.bytecode.object,
      account: walletProvider.account
    });
    const receipt = await (0, import_actions3.getTransactionReceipt)(walletProvider, { hash });
    return (0, import_viem11.getContractAddress)({ from: receipt.from, nonce: receipt.nonce });
  };
  warpTime = async (fork, timestamp) => {
    const client = (0, import_viem11.createClient)({
    ...
    if (timestamp > currentBlock.timestamp) {
      ...
      await client.request({
        ...
        params: [(0, import_viem11.toHex)(timestamp - currentBlock.timestamp)]
      });
    } else {
      logWarning(
        "tenderly",
  ...
  warpBlocks = async (fork, blockNumber) => {
    ...
    if (blockNumber > currentBlock.number) {
      ...
      await client.request({
        ...
        params: [(0, import_viem11.toHex)(blockNumber - currentBlock.number)]
      });
    } else {
      logWarning(
        "tenderly",
  ...
  unwrapAndExecuteSimulationPayloadOnFork = async (fork, request) => {
    ...
    const publicProvider = (0, import_viem11.createClient)({
      chain: { id: fork.forkNetworkId },
      transport: (0, import_viem11.http)(fork.forkUrl)
    });
    if (request.state_objects) {
        ...
        if (request.state_objects[address].storage) {
          for (const slot of Object.keys(request.state_objects[address].storage)) {
            await publicProvider.request({
              ...
              params: [
                address,
                (0, import_viem11.pad)(slot, { size: 32 }),
                (0, import_viem11.pad)(request.state_objects[address].storage[slot], {
                  size: 32
                })
              ]
            });
    ...
    if (request.block_header?.timestamp) {
      await this.warpTime(fork, (0, import_viem11.fromHex)(request.block_header?.timestamp, "bigint"));
    }
    if (request.block_header?.number) {
      await this.warpBlocks(fork, (0, import_viem11.fromHex)(request.block_header?.number, "bigint"));
    }
    if (request.input) {
      ...
      const hash = await (0, import_actions3.sendTransaction)(walletProvider, {
        data: request.input,
        to: request.to,
...
var HUMAN_READABLE_PAYLOAD_STATE = {
  [import_aave_v3_governance_cache.PayloadState.None]: "None",
  [import_aave_v3_governance_cache.PayloadState.Created]: "Created",
  [import_aave_v3_governance_cache.PayloadState.Queued]: "Queued",
  [import_aave_v3_governance_cache.PayloadState.Executed]: "Executed",
  [import_aave_v3_governance_cache.PayloadState.Cancelled]: "Cancelled",
  [import_aave_v3_governance_cache.PayloadState.Expired]: "Expired"
};
var SLOTS = {
  PAYLOADS_MAPPING: 3n
};
var getPayloadsController = (address, client) => {
  const controllerContract = (0, import_viem12.getContract)({
    abi: import_abis2.IPayloadsControllerCore_ABI,
    address,
    client
  });
  const getSimulationPayloadForExecution = async (id) => {
    ...
    const simulationPayload = {
      ...
      input: (0, import_viem12.encodeFunctionData)({
        abi: import_abis2.IPayloadsControllerCore_ABI,
      ...
      state_objects: {
        [controllerContract.address]: {
          storage: {
            [getSolidityStorageSlotUint(SLOTS.PAYLOADS_MAPPING, BigInt(id))]: (0, import_viem12.encodePacked)(
              ...
              [
                ...
                payload.createdAt,
                import_aave_v3_governance_cache.PayloadState.Queued,
...
async function getProof(client, address, slots, blockHash) {
  const block = await (0, import_actions5.getBlock)(client, { blockHash });
  return (0, import_actions5.getProof)(client, {
    address,
...
var getAccountRPL = (proof) => {
  return (0, import_viem13.toRlp)(proof.map((rpl) => (0, import_viem13.fromRlp)(rpl, "hex")));
};
...
var HUMAN_READABLE_STATE = {
  [import_aave_v3_governance_cache2.ProposalState.Null]: "Null",
  [import_aave_v3_governance_cache2.ProposalState.Created]: "Created",
  [import_aave_v3_governance_cache2.ProposalState.Active]: "Active",
  [import_aave_v3_governance_cache2.ProposalState.Queued]: "Queued",
  [import_aave_v3_governance_cache2.ProposalState.Executed]: "Executed",
  [import_aave_v3_governance_cache2.ProposalState.Failed]: "Failed",
  [import_aave_v3_governance_cache2.ProposalState.Cancelled]: "Cancelled",
  [import_aave_v3_governance_cache2.ProposalState.Expired]: "Expired"
};
var getGovernance = ({ address, client }) => {
  const governanceContract = (0, import_viem14.getContract)({
    abi: import_abis3.IGovernanceCore_ABI,
    address,
    client
  });
  ...
  async function getSimulationPayloadForExecution(proposalId) {
    const currentBlock = await (0, import_actions6.getBlock)(client);
    const proposalSlot = getSolidityStorageSlotUint(SLOTS2.PROPOSALS_MAPPING, proposalId);
    const data = await (0, import_actions6.getStorageAt)(client, {
      address: governanceContract.address,
      slot: proposalSlot
    });
    let proposalSlot1 = (0, import_viem14.fromHex)(data, { to: "bigint" });
    proposalSlot1 = setBits(proposalSlot1, 0n, 8n, import_aave_v3_governance_cache2.ProposalState.Queued);
    ...
    const simulationPayload = {
      ...
      input: (0, import_viem14.encodeFunctionData)({
        abi: import_abis3.IGovernanceCore_ABI,
      ...
      state_objects: {
        [governanceContract.address]: {
          storage: {
            [proposalSlot]: (0, import_viem14.toHex)(proposalSlot1, { size: 32 })
            // state & time
  ...
  return {
    ...
    async simulateProposalExecutionOnTenderly(proposalId, { executedLog }) {
      if (executedLog) {
        const tx = await (0, import_actions6.getTransaction)(client, {
          hash: executedLog.transactionHash
        });
        return tenderly.simulateTx(client.chain.id, tx);
      }
        ...
        getProof(
          ...
          [
            getSolidityStorageSlotBytes(
              ...
              (0, import_viem14.toHex)(votingChainId, { size: 32 })
            )
          ],
          proposal.snapshotBlockHash
        )
        ...
        Object.keys(addresses2).map(
          (address2) => getProof(
            ...
            Object.keys(addresses2[address2]).map(
              (slotKey) => (0, import_viem14.toHex)(addresses2[address2][slotKey])
            ),
            proposal.snapshotBlockHash
          )
        )
