...
var import_js_utils = require("@bgd-labs/js-utils");
...
function getSolidityStorageSlotBytes(mappingSlot, key) {
  const slot = (0, import_viem.pad)(mappingSlot, { size: 32 });
  return (0, import_viem.trim)(
    (0, import_viem.keccak256)((0, import_viem.encodeAbiParameters)((0, import_viem.parseAbiParameters)("bytes32, uint256"), [key, BigInt(slot)]))
  );
}
function getSolidityStorageSlotUint(mappingSlot, key) {
  return (0, import_viem.keccak256)((0, import_viem.encodeAbiParameters)((0, import_viem.parseAbiParameters)("uint256, uint256"), [key, mappingSlot]));
}
function getSolidityStorageSlotAddress(mappingSlot, key) {
  return (0, import_viem.keccak256)(
    (0, import_viem.encodeAbiParameters)((0, import_viem.parseAbiParameters)("address, uint256"), [key, BigInt(mappingSlot)])
  );
}
...
function renderEModeValue(key, emode, snapshot) {
  ...
  if (["reserveFactor", "liquidationProtocolFee", "liquidationThreshold", "ltv"].includes(key))
    return `${(0, import_viem2.formatUnits)(BigInt(emode[key]), 2)} %`;
...
var addresses = __toESM(require("@bgd-labs/aave-address-book"), 1);
var import_abis = require("@bgd-labs/aave-address-book/abis");
var import_find_object_paths = require("find-object-paths");
...
function isKnownAddress(value, chainId) {
  ...
  const results = (0, import_find_object_paths.findObjectPaths)(transformedAddresses, { value: (0, import_viem3.getAddress)(value) });
...
async function findAsset(client, address) {
  ...
  const erc20Contract = (0, import_viem3.getContract)({ client, address, abi: import_abis.IERC20Detailed_ABI });
  let symbol = "unknown";
  let decimals = 0;
  try {
...
async function assetIndexesToAsset(client, poolAddress, indexes) {
  if (!cachedReservesList[client.chain.id])
    cachedReservesList[client.chain.id] = await (0, import_viem3.getContract)({
      client,
      abi: import_abis.IPool_ABI,
      address: poolAddress
...
function prettifyNumber({
  ...
  const formattedNumber = limitDecimalsWithoutRounding(
    formatNumberString((0, import_viem4.formatUnits)(BigInt(patchedValue || value), decimals)),
    4
  );
...
async function addAssetPrice(client, address) {
  const clProxy = (0, import_viem4.getContract)({ client, address, abi: CL_PROXY_ABI });
...
var import_rpc_env = require("@bgd-labs/rpc-env");
var import_viem5 = require("viem");
function getClient(chainId) {
  const chain = import_rpc_env.ChainList[chainId];
  if (!chain) return;
  return (0, import_viem5.createClient)({
    transport: (0, import_viem5.http)(
      (0, import_rpc_env.getRPCUrl)(chainId, {
...
function renderReserveValue(key, reserve, chainId) {
  ...
  if (key === "oracleLatestAnswer" && reserve.oracleDecimals)
    return (0, import_viem6.formatUnits)(BigInt(reserve[key]), reserve.oracleDecimals);
...
function renderStrategyValue(key, reserve) {
  ...
  return `${(0, import_viem7.formatUnits)(BigInt(reserve[key]), 25)} %`;
}
var ORDER3 = [
  "optimalUsageRatio",
...
function readJsonString(content) {
  return JSON.parse(JSON.stringify((0, import_json_bigint.default)({ storeAsString: true }).parse(content)));
}
function readJsonFile(filePath) {
  const content = import_node_fs.default.readFileSync(import_node_path.default.join(process.cwd(), filePath), "utf8");
...
function addCommand(program2) {
  program2.command("diff-snapshots").description("generate a snapshot diff report").argument("<from>").argument("<to>").option("-o, --out <string>", "output path").option("--stringMode", "expects input to be a string, not paths").action(async (_from, _to, options) => {
    ...
    if (options.out) {
      import_node_fs2.default.mkdirSync(import_node_path2.default.dirname(options.out), { recursive: true });
      import_node_fs2.default.writeFileSync(options.out, content);
    } else {
      console.log(content);
    }
  });
  ...
  program2.command("diff-storage").description("generate a storage diff").argument("<from>").argument("<to>").option("-o, --out <string>", "output path").action(async (_from, _to, options) => {
    ...
    const content = await (0, import_js_utils.compareStorageLayouts)(from, to);
    if (options.out) {
      import_node_fs2.default.mkdirSync(import_node_path2.default.dirname(options.out), { recursive: true });
      import_node_fs2.default.writeFileSync(options.out, content);
    } else {
...
var import_aave_address_book3 = require("@bgd-labs/aave-address-book");
var import_abis2 = require("@bgd-labs/aave-address-book/abis");
...
var DEFAULT_GOVERNANCE = import_aave_address_book.GovernanceV3Ethereum.GOVERNANCE;
  ...
  simulate = async (request, client) => {
    ...
    if (!request.state_objects[request.from]) {
      request.state_objects[request.from] = {
        balance: String((0, import_viem8.parseEther)("3"))
      };
    } else {
      request.state_objects[request.from].balance = String((0, import_viem8.parseEther)("3"));
    }
...
`
  ...
  deployCode = async (fork, filePath, from) => {
    const walletProvider = (0, import_viem8.createClient)({
      account: from || EOA,
      chain: { id: fork.forkNetworkId, name: "tenderly" },
      transport: (0, import_viem8.http)(fork.forkUrl)
    });
    const artifact = require(filePath);
    logInfo("tenderly", `deploying ${filePath}`);
    const hash = await (0, import_actions.deployContract)(walletProvider, {
      abi: artifact.abi,
      bytecode: artifact.bytecode.object,
      account: walletProvider.account
    });
    const receipt = await (0, import_actions.getTransactionReceipt)(walletProvider, { hash });
    return (0, import_viem8.getContractAddress)({ from: receipt.from, nonce: receipt.nonce });
  };
  warpTime = async (fork, timestamp) => {
    const client = (0, import_viem8.createClient)({
    ...
    if (timestamp > currentBlock.timestamp) {
      ...
      await client.request({
        ...
        params: [(0, import_viem8.toHex)(timestamp - currentBlock.timestamp)]
      });
    } else {
      logWarning(
        "tenderly",
  ...
  warpBlocks = async (fork, blockNumber) => {
    ...
    if (blockNumber > currentBlock.number) {
      ...
      await client.request({
        ...
        params: [(0, import_viem8.toHex)(blockNumber - currentBlock.number)]
      });
    } else {
      logWarning(
        "tenderly",
  ...
  unwrapAndExecuteSimulationPayloadOnFork = async (fork, request) => {
    ...
    const publicProvider = (0, import_viem8.createClient)({
      chain: { id: fork.forkNetworkId },
      transport: (0, import_viem8.http)(fork.forkUrl)
    });
    if (request.state_objects) {
        ...
        if (request.state_objects[address].storage) {
          for (const slot of Object.keys(request.state_objects[address].storage)) {
            await publicProvider.request({
              ...
              params: [
                address,
                (0, import_viem8.pad)(slot, { size: 32 }),
                (0, import_viem8.pad)(request.state_objects[address].storage[slot], {
                  size: 32
                })
              ]
            });
    ...
    if (request.block_header?.timestamp) {
      await this.warpTime(fork, (0, import_viem8.fromHex)(request.block_header?.timestamp, "bigint"));
    }
    if (request.block_header?.number) {
      await this.warpBlocks(fork, (0, import_viem8.fromHex)(request.block_header?.number, "bigint"));
    }
    if (request.input) {
      ...
      const hash = await (0, import_actions.sendTransaction)(walletProvider, {
        data: request.input,
        to: request.to,
...
// src/govv3/proofs.ts
var import_aave_address_book2 = require("@bgd-labs/aave-address-book");
var import_viem9 = require("viem");
var import_actions2 = require("viem/actions");
var WAREHOUSE_SLOTS = {
  [import_aave_address_book2.AaveSafetyModule.STK_AAVE]: {
    exchangeRate: 81n
    // exchangeRate
  }
};
var VOTING_SLOTS = {
  [import_aave_address_book2.AaveSafetyModule.STK_AAVE]: { balance: 0n },
  // balance
  [import_aave_address_book2.AaveV3Ethereum.ASSETS.AAVE.A_TOKEN]: {
    balance: 52n,
    // balance
    delegation: 64n
  },
  // delegation
  [import_aave_address_book2.AaveV3Ethereum.ASSETS.AAVE.UNDERLYING]: { balance: 0n },
  // balance
  [import_aave_address_book2.GovernanceV3Ethereum.GOVERNANCE]: { representative: 9n }
  // representative
};
async function getProof(client, address, slots, blockHash) {
  const block = await (0, import_actions2.getBlock)(client, { blockHash });
  return (0, import_actions2.getProof)(client, {
    address,
...
var getBlockRLP = (rawBlock) => {
  const rawData = [
    ...
    (0, import_viem9.toHex)(rawBlock.number || 0),
    // 0 to account for type null
    (0, import_viem9.toHex)(rawBlock.gasLimit),
    (0, import_viem9.toHex)(rawBlock.gasUsed),
    (0, import_viem9.toHex)(rawBlock.timestamp),
    rawBlock.extraData,
    rawBlock.mixHash,
    rawBlock.nonce,
    (0, import_viem9.toHex)(rawBlock.baseFeePerGas || 0),
    // 0 to account for type null
    // @ts-ignore looks like this field is not yet added into the type
    rawBlock.withdrawalsRoot
  ];
  return (0, import_viem9.toRlp)(rawData);
};
var getAccountRPL = (proof) => {
  return (0, import_viem9.toRlp)(proof.map((rpl) => (0, import_viem9.fromRlp)(rpl, "hex")));
};

// src/govv3/governance.ts
var import_aave_v3_governance_cache = require("@bgd-labs/aave-v3-governance-cache");
var SLOTS = {
  PROPOSALS_MAPPING: 7n
};
var HUMAN_READABLE_STATE = {
  [import_aave_v3_governance_cache.ProposalState.Null]: "Null",
  [import_aave_v3_governance_cache.ProposalState.Created]: "Created",
  [import_aave_v3_governance_cache.ProposalState.Active]: "Active",
  [import_aave_v3_governance_cache.ProposalState.Queued]: "Queued",
  [import_aave_v3_governance_cache.ProposalState.Executed]: "Executed",
  [import_aave_v3_governance_cache.ProposalState.Failed]: "Failed",
  [import_aave_v3_governance_cache.ProposalState.Cancelled]: "Cancelled",
  [import_aave_v3_governance_cache.ProposalState.Expired]: "Expired"
};
var getGovernance = ({ address, client }) => {
  const governanceContract = (0, import_viem10.getContract)({
    abi: import_abis2.IGovernanceCore_ABI,
    address,
    client
  });
  ...
  async function getSimulationPayloadForExecution(proposalId) {
    ...
    const data = await (0, import_actions3.getStorageAt)(client, {
      address: governanceContract.address,
      slot: proposalSlot
    });
    let proposalSlot1 = (0, import_viem10.fromHex)(data, { to: "bigint" });
    proposalSlot1 = setBits(proposalSlot1, 0n, 8n, import_aave_v3_governance_cache.ProposalState.Queued);
    ...
    const simulationPayload = {
      ...
      input: (0, import_viem10.encodeFunctionData)({
        abi: import_abis2.IGovernanceCore_ABI,
      ...
      state_objects: {
        [governanceContract.address]: {
          storage: {
            [proposalSlot]: (0, import_viem10.toHex)(proposalSlot1, { size: 32 })
            // state & time
  ...
  return {
    ...
    async simulateProposalExecutionOnTenderly(proposalId, { executedLog }) {
      if (executedLog) {
        const tx = await (0, import_actions3.getTransaction)(client, {
    ...
    async getVotingProofs(proposalId, voter, votingChainId) {
      ...
      const [stkAaveProof, aaveProof, aAaveProof, representativeProof] = await Promise.all([
        getProof(
          client,
          import_aave_address_book3.AaveSafetyModule.STK_AAVE,
          [getSolidityStorageSlotAddress(VOTING_SLOTS[import_aave_address_book3.AaveSafetyModule.STK_AAVE].balance, voter)],
          proposal.snapshotBlockHash
        ),
        getProof(
          client,
          import_aave_address_book3.AaveV3Ethereum.ASSETS.AAVE.UNDERLYING,
          [
            getSolidityStorageSlotAddress(
              VOTING_SLOTS[import_aave_address_book3.AaveV3Ethereum.ASSETS.AAVE.UNDERLYING].balance,
              voter
            )
          ],
          proposal.snapshotBlockHash
        ...
        getProof(
          client,
          import_aave_address_book3.AaveV3Ethereum.ASSETS.AAVE.A_TOKEN,
          [
            getSolidityStorageSlotAddress(
              VOTING_SLOTS[import_aave_address_book3.AaveV3Ethereum.ASSETS.AAVE.A_TOKEN].balance,
              voter
            ),
            getSolidityStorageSlotAddress(
              VOTING_SLOTS[import_aave_address_book3.AaveV3Ethereum.ASSETS.AAVE.A_TOKEN].delegation,
              voter
            )
          ],
          proposal.snapshotBlockHash
        ...
        getProof(
          client,
          import_aave_address_book3.GovernanceV3Ethereum.GOVERNANCE,
          [
            getSolidityStorageSlotBytes(
              getSolidityStorageSlotAddress(
                VOTING_SLOTS[import_aave_address_book3.GovernanceV3Ethereum.GOVERNANCE].representative,
                voter
              ),
              (0, import_viem10.toHex)(votingChainId, { size: 32 })
            )
          ],
          proposal.snapshotBlockHash
        )
    ...
    async getStorageRoots(proposalId) {
      const proposal = await getProposal(proposalId);
      const addresses2 = (0, import_deepmerge.default)(VOTING_SLOTS, WAREHOUSE_SLOTS);
        ...
        Object.keys(addresses2).map(
          (address2) => getProof(
            ...
            Object.keys(addresses2[address2]).map(
              (slotKey) => (0, import_viem10.toHex)(addresses2[address2][slotKey])
            ),
            proposal.snapshotBlockHash
          )
        )
...
var import_abis3 = require("@bgd-labs/aave-address-book/abis");
var import_viem11 = require("viem");
var import_actions4 = require("viem/actions");
var import_aave_v3_governance_cache2 = require("@bgd-labs/aave-v3-governance-cache");
var HUMAN_READABLE_PAYLOAD_STATE = {
  [import_aave_v3_governance_cache2.PayloadState.None]: "None",
  [import_aave_v3_governance_cache2.PayloadState.Created]: "Created",
  [import_aave_v3_governance_cache2.PayloadState.Queued]: "Queued",
  [import_aave_v3_governance_cache2.PayloadState.Executed]: "Executed",
  [import_aave_v3_governance_cache2.PayloadState.Cancelled]: "Cancelled",
  [import_aave_v3_governance_cache2.PayloadState.Expired]: "Expired"
};
var SLOTS2 = {
  PAYLOADS_MAPPING: 3n
};
var getPayloadsController = (address, client) => {
  const controllerContract = (0, import_viem11.getContract)({
    abi: import_abis3.IPayloadsControllerCore_ABI,
    address,
    client
  });
  const getSimulationPayloadForExecution = async (id) => {
    ...
    const simulationPayload = {
      ...
      input: (0, import_viem11.encodeFunctionData)({
        abi: import_abis3.IPayloadsControllerCore_ABI,
      ...
      state_objects: {
        [controllerContract.address]: {
          storage: {
            [getSolidityStorageSlotUint(SLOTS2.PAYLOADS_MAPPING, BigInt(id))]: (0, import_viem11.encodePacked)(
              ...
              [
                ...
                payload.createdAt,
                import_aave_v3_governance_cache2.PayloadState.Queued,
  ...
  return {
    ...
    simulatePayloadExecutionOnTenderly: async (id, { executedLog }) => {
      if (executedLog) {
        const tx = await (0, import_actions4.getTransaction)(client, {
          hash: executedLog.transactionHash
        });
        return tenderly.simulateTx(client.chain.id, tx);
      }
...
function addCommand2(program2) {
  program2.command("fork").description("generates a fork (optionally with a proposal executeds").requiredOption("--chainId <number>", "the chainId to fork offs").option("--blockNumber <number>", "the blocknumber to fork of (latest if omitted)").option("--alias <string>", "Set a custom alias").option("--proposalId <number>", "ProposalId to execute").option("--payloadId <number>", "PayloadId to execute").option("--payloadAddress <string>", "Payload address").option("--artifactPath <string>", "path to the local payload").action(async (options) => {
    ...
    if (artifactPath) {
      ...
      const payload = await tenderly.deployCode(fork, import_node_path3.default.join(process.cwd(), artifactPath));
      const walletProvider = (0, import_viem12.createWalletClient)({
        account: EOA,
        chain: { id: fork.forkNetworkId, name: "tenderly" },
        transport: (0, import_viem12.http)(fork.forkUrl)
      });
...
var import_abis4 = require("@bgd-labs/aave-address-book/abis");
...
function getContractName(contracts, address, chainId) {
  ...
  if (!contract)
    return `unknown contract name at \`${(0, import_viem13.getAddress)(address)}\`${flagKnownAddress(isAddrKnown)}`;
  ...
  if (contract.standards?.includes("eip1967") && contract.child_contracts?.[0].address) {
    return `${contractName} at \`${(0, import_viem13.getAddress)(address)}\`${flagKnownAddress(
      isAddrKnown
    )} with implementation ${getContractName(contracts, contract.child_contracts?.[0].address, chainId)}`;
  }
  return `${contractName} at \`${(0, import_viem13.getAddress)(address)}\`${flagKnownAddress(isAddrKnown)}`;
}
...
var checkLogs = {
  ...
  async checkProposal(proposal, sim, client) {
    ...
    const events = sim.transaction.transaction_info.logs?.reduce(
      (logs, log) => {
        const addr = (0, import_viem14.getAddress)(log.raw.address);
...
async function checkNoSelfdestruct(trustedAddrs, addr, client) {
  ...
  const [code, nonce] = await Promise.all([
    (0, import_actions5.getBytecode)(client, { address: addr }),
    (0, import_actions5.getTransactionCount)(client, { address: addr })
  ]);
...
var pools = __toESM(require("@bgd-labs/aave-address-book"), 1);
    ...
    if (!simulation.transaction.status) {
      ...
      const stateDiffs = simulation.transaction.transaction_info.state_diff.reduce(
        (diffs, diff2) => {
          ...
          const addr = (0, import_viem15.getAddress)(diff2.raw[0].address);
...
async function checkVerificationStatus(sim, addr, client) {
  ...
  const code = await (0, import_actions6.getBytecode)(client, { address: addr });
...
var import_refreshCache = require("@bgd-labs/aave-v3-governance-cache/refreshCache");
var import_customStorageProvider = require("@bgd-labs/aave-v3-governance-cache/customStorageProvider");
var import_fileSystemStorageAdapter = require("@bgd-labs/aave-v3-governance-cache/fileSystemStorageAdapter");
var localCacheAdapter = (0, import_customStorageProvider.customStorageProvider)(import_fileSystemStorageAdapter.fileSystemStorageAdapter);
async function simulateProposal(governanceAddress, client, proposalId) {
  ...
  await (0, import_refreshCache.refreshCache)(localCacheAdapter);
...
var import_customStorageProvider2 = require("@bgd-labs/aave-v3-governance-cache/customStorageProvider");
var import_fileSystemStorageAdapter2 = require("@bgd-labs/aave-v3-governance-cache/fileSystemStorageAdapter");
var import_refreshCache2 = require("@bgd-labs/aave-v3-governance-cache/refreshCache");
var localCacheAdapter2 = (0, import_customStorageProvider2.customStorageProvider)(import_fileSystemStorageAdapter2.fileSystemStorageAdapter);
function addCommand3(program2) {
  ...
  govV3.command("simulate-payload").description("simulates a payloadId on tenderly").requiredOption("--chainId <number>", "the chainId to fork of").requiredOption("--payloadId <number>", "payloadId to simulate via tenderly").action(async ({ payloadId: _payloadId, chainId }, options) => {
    await (0, import_refreshCache2.refreshCache)(localCacheAdapter2);
  ...
  govV3.command("view").description("shows all the proposals & state").action(async (opts) => {
    ...
    let exitLvl2 = false;
    while (!exitLvl2) {
      const moreInfo = await (0, import_prompts.select)({
      ...
      if (moreInfo === 3 /* HOW_TO_VOTE */) {
        const address = await (0, import_prompts.input)({
          message: "Enter the address you would like to vote with"
        });
        const support = await (0, import_prompts.confirm)({
          message: "Are you in Support of the proposal?"
        });
        const portal = (0, import_viem16.getContract)({
          address: cache.proposal.votingPortal,
          abi: import_abis4.IVotingPortal_ABI,
          client: DEFAULT_GOVERNANCE_CLIENT
        });
        ...
        if (proofs.length === 0) logError("Voting Error", "You need voting power to vote");
          ...
          if (FORMAT === "raw") {
            ...
            logSuccess(
              "encoded calldata",
              (0, import_viem16.encodeFunctionData)({
                abi: import_abis4.IVotingMachineWithProofs_ABI,
      ...
      if (moreInfo === 4 /* HOW_TO_REGISTER_STORAGE_ROOTS */) {
        const portalContract = (0, import_viem16.getContract)({
          address: cache.proposal.votingPortal,
          abi: import_abis4.IVotingPortal_ABI,
          client: DEFAULT_GOVERNANCE_CLIENT
        });
        ...
        const machineContract = (0, import_viem16.getContract)({
          address: machine,
          abi: import_abis4.IVotingMachineWithProofs_ABI,
        ...
        const block = await (0, import_actions7.getBlock)(DEFAULT_GOVERNANCE_CLIENT, {
        ...
        if (FORMAT === "raw") {
          ...
          roots.map((root, ix) => {
            ...
            logSuccess(
              "Encoded callData",
              (0, import_viem16.encodeFunctionData)({
                abi: import_abis4.IDataWarehouse_ABI,
...
var aipType = import_zod.z.object({
  discussions: import_zod.z.string(),
  title: import_zod.z.string(),
  author: import_zod.z.string(),
  snapshot: import_zod.z.string().optional()
});
...
var import_ipfs_unixfs_importer = require("ipfs-unixfs-importer");
...
var Hash = {
  of: async (content, options = {}) => {
    ...
    let lastCid;
    for await (const { cid } of (0, import_ipfs_unixfs_importer.importer)([{ content }], new import_blockstore_core.MemoryBlockstore(), {
      cidVersion: 0,
      rawLeaves: false
    })) {
      lastCid = cid;
...
function addCommand4(program2) {
  program2.command("ipfs").description("generates the ipfs hash for specified source").argument("<source>").option("-u, --upload").option("--verbose").action(async (source, { upload, verbose }) => {
    const filePath = import_node_path4.default.join(process.cwd(), source);
...
async function generateCapoReport(snapshot) {
  ...
  for (const key in snapshot.prices) {
    ...
    const sourcePrice = (0, import_viem17.formatUnits)(BigInt(price.sourcePrice), snapshot.decimals);
    const referencePrice = (0, import_viem17.formatUnits)(BigInt(price.referencePrice), snapshot.decimals);
...
function addCommand5(program2) {
  program2.command("capo-report").description("generate a capo report").argument("<source>").option("-o, --out <string>", "output path").option("--stringMode", "expects input to be a string, not paths").action(async (_source, options) => {
    ...
    if (options.out) {
      import_node_fs4.default.writeFileSync(options.out, content);
    } else {
      console.log(content);
    }
  });
...
// src/cli.ts
var program = new import_extra_typings.Command();
program.name("aave-cli").description("CLI to interact with the aave ecosystem").option("-v, --verbose", "Showing logs for all the taken steps").on("option:verbose", () => {
  ...
  new import_extra_typings.Option("--format <format>", "Set preferred output format").default("raw").choices(["raw", "encoded"])
