...
contract PoolInstanceWithCustomInitialize is PoolInstance {
  ...
  function initialize(IPoolAddressesProvider provider) external virtual override initializer {
    ...
    // @note This specific logic block should only be executed for the Ethereum Mainnet Aave V3 Core Pool instance.
    //       The combined check of `address(this)` and `block.chainid` is sufficient because
    //       the Ethereum Aave V3 Core Pool address is unique, and this ensures execution only on Mainnet.
