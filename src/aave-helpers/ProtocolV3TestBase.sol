...
import {
  ProtocolV3TestBase as RawProtocolV3TestBase, ReserveConfig
} from "aave-v3-origin-tests/utils/ProtocolV3TestBase.sol";

import {IInitializableAdminUpgradeabilityProxy} from
  "aave-helpers/src/interfaces/IInitializableAdminUpgradeabilityProxy.sol";
import {ExtendedAggregatorV2V3Interface} from "aave-helpers/src/interfaces/ExtendedAggregatorV2V3Interface.sol";
import {CommonTestBase, ReserveTokens, ChainIds} from "aave-helpers/src/CommonTestBase.sol";
import {ILegacyDefaultInterestRateStrategy} from "aave-helpers/src/dependencies/ILegacyDefaultInterestRateStrategy.sol";
import {MockFlashLoanReceiver} from "aave-helpers/src/mocks/MockFlashLoanReceiver.sol";
...
contract ProtocolV3TestBase is RawProtocolV3TestBase, CommonTestBase {
   ...
   */
  function defaultTest(string memory reportName, IPool pool, address payload)
    public
    returns (ReserveConfig[] memory, ReserveConfig[] memory)
  {
    return defaultTest(reportName, pool, payload, true);
  }

  function defaultTest(string memory reportName, IPool pool, address payload, bool runE2E)
    public
    returns (ReserveConfig[] memory, ReserveConfig[] memory)
  {
  ...
  function configChangePlausibilityTest(ReserveConfig[] memory configBefore, ReserveConfig[] memory configAfter)
    public
    view
  {
    ...
    for (uint256 i = 0; i < configAfter.length; i++) {
      ...
      if (i < configsBeforeLength) {
        ...
        require(
          configAfter[i].supplyCap / 1e2 <= totalSupply / IERC20Metadata(configAfter[i].underlying).decimals(),
          "PL_SUPPLY_CAP_GT_TOTAL_SUPPLY"
        );
      }
      // borrow cap should never exceed supply cap
      if (
        configAfter[i].borrowCap != 0 && configAfter[i].underlying != AaveV3EthereumAssets.GHO_UNDERLYING // GHO is the exclusion from the rule
      ) {
  ...
  function e2eTestAsset(IPool pool, ReserveConfig memory collateralConfig, ReserveConfig memory testAssetConfig) public {
    console.log("E2E: Collateral %s, TestAsset %s", collateralConfig.symbol, testAssetConfig.symbol);
    ...
    // test deposits and withdrawals
    {
      ...
      poolConfigurator.setSupplyCap(testAssetConfig.underlying, aTokenTotalSupply / 10 ** testAssetConfig.decimals + 1);
      vm.prank(addressesProvider.getACLAdmin());
      poolConfigurator.setBorrowCap(
        testAssetConfig.underlying, variableDebtTokenTotalSupply / 10 ** testAssetConfig.decimals + 1
      );
  ...
  function _changeAssetPrice(IPool pool, ReserveConfig memory config, uint256 assetPriceChangePercentage) internal {
    IPoolAddressesProvider addressesProvider = IPoolAddressesProvider(pool.ADDRESSES_PROVIDER());
  ...
  function _setAssetLtvAndLiquidationThreshold(
    ...
    vm.prank(addressesProvider.getACLAdmin());
    poolConfigurator.configureReserveAsCollateral(config.underlying, newLtv, newLiquidationThreshold, 105_00);
  }

  /**
   * @dev returns a "good" collateral in the list
   */
  function _getGoodCollateral(ReserveConfig[] memory configs) private pure returns (ReserveConfig memory config) {
    for (uint256 i = 0; i < configs.length; i++) {
      if (
        ...
        // ltv is not 0
        _includeInE2e(configs[i]) && configs[i].usageAsCollateralEnabled && configs[i].debtCeiling == 0
          && configs[i].ltv != 0
      ) return configs[i];
    }
  ...
  function _deposit(ReserveConfig memory config, IPool pool, address user, uint256 amount) internal {
  ...
  function _withdraw(ReserveConfig memory config, IPool pool, address user, uint256 amount) internal returns (uint256) {
    vm.startPrank(user);
    uint256 aTokenBefore = IERC20(config.aToken).balanceOf(user);
  ...
  function _repay(ReserveConfig memory config, IPool pool, address user, uint256 amount, bool withATokens) internal {
    vm.startPrank(user);
  ...
  function _liquidationCall(
    ...
    console.log("LIQUIDATE: %s, Amount: %s, Debt Amount: %s", debtConfig.symbol, debtToCover, debtBefore);
  ...
  function _writeStrategyConfigs(string memory path, ReserveConfig[] memory configs) internal virtual override {
    // keys for json stringification
    ...
    for (uint256 i = 0; i < configs.length; i++) {
      IDefaultInterestRateStrategyV2 strategyV2 = IDefaultInterestRateStrategyV2(configs[i].interestRateStrategy);
      ILegacyDefaultInterestRateStrategy strategyV1 =
        ILegacyDefaultInterestRateStrategy(configs[i].interestRateStrategy);
      ...
      try strategyV1.getVariableRateSlope1() {
        vm.serializeString(key, "baseStableBorrowRate", vm.toString(strategyV1.getBaseStableBorrowRate()));
        ...
        vm.serializeString(key, "baseVariableBorrowRate", vm.toString(strategyV1.getBaseVariableBorrowRate()));
        vm.serializeString(key, "variableRateSlope1", vm.toString(strategyV1.getVariableRateSlope1()));
        vm.serializeString(key, "variableRateSlope2", vm.toString(strategyV1.getVariableRateSlope2()));
        vm.serializeString(
          key, "optimalStableToTotalDebtRatio", vm.toString(strategyV1.OPTIMAL_STABLE_TO_TOTAL_DEBT_RATIO())
        );
        vm.serializeString(
          key, "maxExcessStableToTotalDebtRatio", vm.toString(strategyV1.MAX_EXCESS_STABLE_TO_TOTAL_DEBT_RATIO())
        );
        vm.serializeString(key, "optimalUsageRatio", vm.toString(strategyV1.OPTIMAL_USAGE_RATIO()));
        object = vm.serializeString(key, "maxExcessUsageRatio", vm.toString(strategyV1.MAX_EXCESS_USAGE_RATIO()));
      } catch {
        vm.serializeString(key, "baseVariableBorrowRate", vm.toString(strategyV2.getBaseVariableBorrowRate(asset)));
        vm.serializeString(key, "variableRateSlope1", vm.toString(strategyV2.getVariableRateSlope1(asset)));
        vm.serializeString(key, "variableRateSlope2", vm.toString(strategyV2.getVariableRateSlope2(asset)));
  ...
  function _validateInterestRateStrategy(
    ...
    ILegacyDefaultInterestRateStrategy strategy = ILegacyDefaultInterestRateStrategy(interestRateStrategyAddress);

    require(address(strategy) == expectedStrategy, "_validateInterestRateStrategy() : INVALID_STRATEGY_ADDRESS");
    ...
    require(
      strategy.OPTIMAL_STABLE_TO_TOTAL_DEBT_RATIO() == expectedStrategyValues.optimalStableToTotalDebtRatio,
